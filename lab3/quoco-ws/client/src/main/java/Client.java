...
public class Client {
    ...
    public static void main(String[] args) {
        try {
            // Delay the connection before starting up
            Thread.sleep(30000);

            // Set the host to 0.0.0.0 if no host is given otherwise set it as the first argument
            ...
            // Forming the URL for the service
            URL wsdl_url = new URL("http://" + host + ":" + port + "/broker?wsdl");

            // Forming the service name
            QName service_name = new QName("http://core.service/", "BrokerService");

            // Creating a service instance
            Service service = Service.create(wsdl_url, service_name);

            // Forming the port name
            QName port_name = new QName("http://core.service/", "BrokerPort");

            // Getting access to the broker service
            QuoterService broker_service = service.getPort(port_name, QuoterService.class);
			...
			for (ClientInfo info : clients) {
				...
				// Retrieve quotations from the broker and print them
				for(Quotation quotation : broker_service.getQuotations(info)) {
					displayQuotation(quotation);
				}
				
				// Print a line between each client
				System.out.println("\n");
			}
